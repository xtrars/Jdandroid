...
class PackageQueriesTest : SchemaDbTest() {
    ...
    fun `requeuePackage stellt nur pausierte, fehlgeschlagene und offline Teile des Pakets zurueck`() {
        ...
        item(8, "h.rar", DownloadStatus.QUEUED, note = "FREE_CAPTCHA")
        item(9, "i.rar", DownloadStatus.QUEUED, note = "FREE_WAIT")
        execute("UPDATE downloads SET attempts = 3, retryAt = 99 WHERE id = 2")
        execute("UPDATE downloads SET retryAt = 5000000 WHERE id = 8")
        execute("UPDATE downloads SET retryAt = 1000100 WHERE id = 9")

        assertEquals(4, execute(PackageQueries.REQUEUE_PACKAGE, "packageId" to 1L, "heldFrom" to 4_000_000L))

        assertEquals(listOf(1L, 2L, 3L, 8L, 9L), ids("SELECT id FROM downloads WHERE status = 'QUEUED' ORDER BY id"))
        assertEquals(null, column(8, "errorMessage"))
        assertEquals("0", column(8, "retryAt"))
        assertEquals("FREE_WAIT", column(9, "errorMessage"))
        assertEquals("1000100", column(9, "retryAt"))
