...
class DownloadGroupingTest {
    ...
    fun eintraegeWerdenIhrenPaketenZugeordnet() {
        ...
        val groups = groupDownloads(listOf(item(10, 2), item(11, 1), item(12, 1)), packages)
    ...
    fun leerePaketeErscheinenNicht() {
        ...
        val groups = groupDownloads(listOf(item(10, 1)), packages)
        assertEquals(listOf("A"), groups.map { it.pkg.name })
    }

    @Test
    fun eintraegeOhnePaketLandenUnterOhnePaket() {
        val groups = groupDownloads(listOf(item(10, null)), emptyList())
        assertEquals(1, groups.size)
        assertEquals("Ohne Paket", groups[0].pkg.name)
    ...
    fun eintraegeMitGeloeschtemPaketBleibenSichtbar() {
        ...
        val groups = groupDownloads(listOf(item(10, 1), item(11, 99), item(12, null)), packages)
